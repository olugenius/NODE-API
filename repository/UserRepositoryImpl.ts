    ...
    try {
      ...
      let response = await new Promise<string>((resolve, reject) => {
        connection?.getConnection(async (err, connection) => {
          ...
          connection?.query(
            `INSERT INTO MailToken(Token,Email,MailFor,Used) VALUES(?,?,?,?) `,
    ...
    try {
      ...
      let result = await new Promise<any>((resolve, reject) => {
        connection?.getConnection((err, connection) => {
          ...
          connection?.query(
            `SELECT * FROM  MailToken WHERE Email = ? and Used=False and MailFor= ?`,
            [email, mailFor],
            (err, data) => {
              connection.release();
              if (err) {
    ...
    let result = await new Promise<string>(async (resolve, reject) => {
      ...
      connection?.getConnection((err, connection) => {
        try {
          ...
          connection?.query(
            `UPDATE MailToken SET Used = ?,UpdatedAt= ? where Email=? and MailFor=?`,
            [true, updatedAt, email, mailFor],
            (err, data) => {
              if (err) {
                console.log("error querying database", err);
                resolve("failed");
        ...
        (resolve, reject) => {
          connection?.getConnection(async(err, connection) => {
            ...
            try{
              ...
              const query1 = `INSERT INTO Users(FirstName,LastName,Phone,Email,UserId,Password,FilePath,Gender,Industry) VALUES(?,?,?,?,?,?,?,?,?)`;
