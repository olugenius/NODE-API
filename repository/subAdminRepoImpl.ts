                ...
                connection?.getConnection(async(err,connection)=>{
                    ...
                    const subAdminId = `subAdmin-${GenerateUniqueId()}`
                        ...
                        //  })
                         ...
                         const message = `Hello ${payload.FirstName} You have been created as a SubAdmin in the VSured App. Please Login with this One time. Password: ${pass}`
                         if (!payload.Email) {
                            await SMSHandler(payload.Phone, message);
                         } else {
                           const emailMessage = `<!DOCTYPE html><html><body><h2>Hello ${payload.FirstName} </h2><p><b>You have been created as a SubAdmin in the VSured App</b></p><p class="demo">Please Login with this One time. <br><br> <b>Password:</b> ${pass}</p></body></html>`;
