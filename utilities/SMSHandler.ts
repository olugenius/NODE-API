...
export default async function(phone:string,message:string):Promise<boolean>{
    try{
        // let isSent = false
        ...
        const data = {
          "to":phone,
          "from":process.env?.SMS_SENDER_ID || 'Vsured',
          "sms":message,
          "type":"plain",
        ...
        let response = await axios.post(`${process.env.SMS_URL}`,data,{headers:{'Content-Type':'application/json'}})
        console.log('SMS RES',response.data)
        ...
        console.log('An error occurred sending SMS',error)
        return false
    }
}
