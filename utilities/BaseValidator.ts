...
[
  ...
  body('Name','Name field is required').notEmpty().trim(),
  body('Phone','Phone field is required').optional().trim().isMobilePhone('any',{strictMode:true}).withMessage('invalid phone number. Phone number must include country code'),
  check('Email').optional().trim().isEmail().withMessage('invalid Email format').normalizeEmail(),
