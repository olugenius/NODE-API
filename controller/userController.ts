        ...
        if(!error.isEmpty()){
          ...
          return;
            ...
            if(response?.length < 1){
               ...
               return res.status(HttpStatus.STATUS_200).json({status:HttpStatus.STATUS_SUCCESS,message:'Login Successful',UserId:response[0]?.UserId,Channel:reqBody.Channel,UserRole:response[0]?.UserRole,accessToken:accessToken,refreshToken:refreshToken})
...
router.post('/register/sendMail',EmailValidator,async(req:Request,res:Response)=>{
  try{
   ...
   if(!emailSMSResult){
    ...
    return res.status(HttpStatus.STATUS_200).json({status:HttpStatus.STATUS_SUCCESS,message:`Token sent successfully Via ${reqBody.Medium}`,Channel:`${reqBody.Channel}`,Medium: `${reqBody.Medium}`})
