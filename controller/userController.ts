 ...
 *   post:
 *     summary: Send Mail for Validation
 *     tags: [User]
 *     requestBody:
 *       required: true
 *       content:
...
router.post("/login", LoginValidator, async (req: Request, res: Response) => {
  try {
    ...
    let response = <registerModel[]>(
      await userRepo.GetUserByEmail(reqBody.Email)
    );
  ...
  //  if(response[0].UserRole !== RolesEnum.SUPER_ADMIN){
    ...
    const refreshToken = await jwtHandler.generateRefreshToken(reqBody.Email);

    return res.status(HttpStatus.STATUS_200).json({
      status: HttpStatus.STATUS_SUCCESS,
      message: "Login Successful",
      UserId: response[0]?.UserId,
      Channel: reqBody.Email,
      IsOnboarded:'True',
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      reqBody.FilePath = req.protocol + '://' + req.get('host') +req?.file?.path.replace('public','')
      const error = validationResult(req);
      if (!error.isEmpty()) {
        res.status(HttpStatus.STATUS_400).json(error.array());
        return;
      ...
      if (userData?.length > 0) {
        return res.status(HttpStatus.STATUS_400).json({
          status: HttpStatus.STATUS_FAILED,
          message: "This Email is already registered on the platform. Please log in instead",
        });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // const message = `Please use the token sent to you for validation: \n Token Value is: ${token.toString()}`;
        ...
        emailSMSResult = await SendMail(`${reqBody.Email}`, emailMessage);
      ...
      return res.status(HttpStatus.STATUS_200).json({
        status: HttpStatus.STATUS_SUCCESS,
        message: `Token sent successfully Via Email`,
        Email: `${reqBody.Email}`,
      });
    } catch (err) {
      console.log("error creating user at controller side", err);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let response = <userToken[]>(
        await userRepo.GetUserToken(reqBody.Email, "EmailVerify")
      );
      if (response?.length < 1) {
        res.status(HttpStatus.STATUS_400).json({
          status: HttpStatus.STATUS_FAILED,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log("An error occurred sending mail", err);
      return res.status(HttpStatus.STATUS_500).json({
        status: HttpStatus.STATUS_500,
        message: "Something went wrong",
      });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      //Get Token from Db
      let response = <userToken[]>(
        await userRepo.GetUserToken(reqBody.Email, "ForgotPassword")
      );
      if (response?.length < 1) {
        res.status(HttpStatus.STATUS_400).json({
          status: HttpStatus.STATUS_FAILED,
...
//     let response = <registerModel[]>(
//       await userRepo.GetUserByEmailOrPhone(reqBody.Email)
//     );
//     if (
//       response?.length > 0
//     ) {
...
router.get("/profile/:email", Authorize, async (req, res) => {
  try {
    ...
    var response = await userRepo.GetUserByEmail(param);
    if (response?.length < 1) {
